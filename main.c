...
void readPort_thread_func(void *arg)
{
	...
	while (1) {
		...
		// 使用select实现串口的多路通信
			fs_sel = select(tty_fd1 + 1, &fs_read, NULL, NULL, &time);
		printf("%d\n", fs_sel);
		fs_sel = 1;
		if (fs_sel) {
			memset(rcv_buf, 0, sizeof(rcv_buf));
			len = read(tty_fd1, rcv_buf, 255);
...
int main(int argc, char *argv[])
{
	...
	while(1)
	{
		...
		tty_fd1 = open(UART_DEVICE1, O_RDWR);
		if (tty_fd1 < 0)
		{
			perror(UART_DEVICE1);
		}
	...
	tcgetattr(tty_fd1, &old_termios);

	//初始化串口
	while(1){
		if(tty_init(tty_fd, 0, B38400) < 0&& tty_init(tty_fd1, 0, B38400) < 0) {
