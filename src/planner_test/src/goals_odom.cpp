...
void amclposCallback(const nav_msgs::Odometry &amcl_pose)
{
  geometry_msgs::PoseStamped pose_stamped;
  double d,x,y,roll,pitch,yaw;
  tf::Quaternion quat;
  ...
  if(fabs(d-last_d)>0.4||fabs(yaw-last_yaw)>0.1)
 {
  ...
  ROS_INFO("x=%.2f y=%.2f yaw=%.2f",x*1.0,y*1.0);
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub = n.subscribe("odom", 1000, amclposCallback);
