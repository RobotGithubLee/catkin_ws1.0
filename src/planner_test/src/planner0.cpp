...
int main(int argc, char **argv)
{
  ...
  float pose_x,last_pose_x=0;
  float pose_y,last_pose_y=0;
  float yaw=0;
  ...
  while(ros::ok())
  {
   ...
   else
   {
    ...
    float diff_angle=atan2(sin(delta_pose[2]),cos(delta_pose[2]))/2;
    if(angle_amclpos2line*angle_goalpos2line>0 &&i>1)
    {
     float dist=PointToSegDist(amcl_pose[1],amcl_pose[2],goals[i-1][0],goals[i-1][1],goals[i][0],goals[i][1]);
      if(dist>0.2||fabs(angle_amclpos2line)>0.1)
      {
    ...
    cmdVel.linear.x=fabs(ww-0.15)*25+0.3;
    if(cmdVel.linear.x>0.75)
       cmdVel.linear.x=0.75;
    else if(cmdVel.linear.x<0.3)
        cmdVel.linear.x=0.3;
   ...
   ROS_INFO("i=%d linear.x=%.2f angular.z=%.4f",i,cmdVel.linear.x,cmdVel.angular.z);
