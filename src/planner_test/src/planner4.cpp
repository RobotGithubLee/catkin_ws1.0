...
void amclposCallback(const nav_msgs::Odometry &amclPose)
{
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub = n.subscribe("odom", 100, amclposCallback);
  ...
  while(ros::ok())
  {
   ...
   else
   {
    ...
    if(d_Now2Amcl<0.02||angel_flag<0 ||u>=1)
    {
      ...
      Px[2]=(amcl_pose[0]+goals[i][0])/2;
      Py[2]=amcl_pose[1]+4*K*beta*d_Now2Amcl*beta*d_Now2Amcl/3/(goals[i][1]-amcl_pose[1])*fabs(goals[i][1]-amcl_pose[1]);
  
      Px[3]=goals[i][0]-beta*d_Now2Amcl*cos(normalize(goals[i][2]));
      Py[3]=goals[i][1]-beta*d_Now2Amcl*sin(normalize(goals[i][2]));
     ...
     while(angel_flag<0)
     {
      ...
      if(u>0.5 &&v_f)
      {
       ...
       K=v_flag*CurvatureOnBezierCeuve(4,u,Px,Py); 
       cmdVel.linear.x=0.3; 
